#!/usr/bin/env python3
"""Install/update Codex hooks used by usage-mgr."""
...
SCRIPT_SUFFIX = "skills/usage-mgr/scripts/usage_hook.py"
HOOK_TIMEOUT = 30
...
def command(path: Path) -> str:
    return f"/usr/bin/python3 {path}"


def desired_hooks(codex_home: Path) -> dict[str, list[dict[str, Any]]]:
    usage_hook = codex_home / SCRIPT_SUFFIX
    if not usage_hook.is_file():
        raise FileNotFoundError(
            f"usage hook not found: {usage_hook}; install usage-mgr under the target Codex home first"
        )
    return {
        "Stop": [
            {
                "type": "command",
                "command": command(usage_hook),
        ...
        "SubagentStop": [
            {
                ...
                "timeout": HOOK_TIMEOUT,
                "async": False,
                "statusMessage": "Recording Codex subagent usage",
            }
        ],
    }
...
def command_matches(item: dict[str, Any], desired_command: str) -> bool:
    ...
    if not isinstance(raw, str):
        return False
    return raw == desired_command or raw.endswith(SCRIPT_SUFFIX)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Install/update usage-mgr Codex hooks.")
